...
class AVLNode:
    ...
    # For each AVLNode, also maintain an integer "num". This should be such that
    # node.num is the number of nodes in the subtree rooted at "node". For example,
    # if node is a leaf node, then node.num shoudl be 1; if node is one of the
    # "empty" nodes with key == None then node.num should be 0.
